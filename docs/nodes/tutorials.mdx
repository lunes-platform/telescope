...
## How to Run a Lunes Node in MAINNET and TESTNET
...
You need **Lunes Node java executable** **[download here](https://github.com/lunes-platform/lunesnode/releases/download/v0.1.4/lunesnode-latest.jar)**

Now you also need to define a configuration file as the name `lunesnode.conf` and put what you want.
...
If you want enable Lunes Node to run forever, save this file in `/etc/systemd/system/` with `lunesnode.service`
...
```
docker run lunesplatform/node:0.1.2-mainnet
```
